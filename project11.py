#create input data
arr=list()
arr.append([8,2,22,97,38,15,00,40,0,75,4,5,7,78,52,12,50,77,91,8])
arr.append([49,49,99,40,17,81,18,57,60,87,17,40,98,43,69,48,04,56,62,00])
arr.append([81,49,31,73,55,79,14,29,93,71,40,67,53,88,30,03,49,13,36,65])
arr.append([52,70,95,23,04,60,11,42,69,24,68,56,01,32,56,71,37,02,36,91])
arr.append([22,31,16,71,51,67,63,89,41,92,36,54,22,40,40,28,66,33,13,80])
arr.append([24,47,32,60,99,03,45,02,44,75,33,53,78,36,84,20,35,17,12,50])
...
arr.append([24,55,58,05,66,73,99,26,97,17,78,78,96,83,14,88,34,89,63,72])
...
#up-left attempt
        try:
            attempt=arr[row-3][col-3]*arr[row-2][col-2]*arr[row-1][col-1]*arr[row][col]
...
#up attempt
        try:
            attempt=arr[row][col-3]*arr[row][col-2]*arr[row][col-1]*arr[row][col]
            if attempt>max_score:
                print row,col
                max_score=attempt
        except IndexError:
...
#right attempt
        try:
            attempt=arr[row-3][col]*arr[row-2][col]*arr[row-1][col]*arr[row][col]
            if attempt>max_score:
                print row,col
                max_score=attempt
        except IndexError:
